...
def build_google_calendar_plan(config: SandboxConfig) -> dict:
    outside = update_scheduling_context(AgentState("sandbox-conv-price", LEONARDO_ORG_ID, "Quanto custa a consulta?"))
    generic = update_scheduling_context(AgentState("sandbox-conv-interest", LEONARDO_ORG_ID, "Estou pensando em marcar semana que vem."))
    explicit = update_scheduling_context(AgentState("sandbox-conv-schedule", LEONARDO_ORG_ID, "Quero marcar uma consulta."))
    return {
        ...
        "testEvent": {
            "summary": "[SANDBOX] Paciente teste - Dr. Leonardo Carvalho",
...
def run_google_calendar_smoke(config: SandboxConfig) -> dict:
    ...
    price = update_scheduling_context(AgentState("sandbox-conv-price", LEONARDO_ORG_ID, "Quanto custa a consulta?"))
    generic = update_scheduling_context(AgentState("sandbox-conv-interest", LEONARDO_ORG_ID, "Estou pensando em marcar semana que vem."))
    ...
    active = update_scheduling_context(AgentState("sandbox-conv-schedule", LEONARDO_ORG_ID, "Tem horario sexta?"))
    ...
    try:
        created = provider.create_appointment(
            ...
            payload={
                "summary": "[SANDBOX] Paciente teste - Dr. Leonardo Carvalho",
                "description": "Evento de teste criado por smoke test sandbox. Pode ser removido.",
        ...
        if created_id:
            provider.cancel_appointment(organization_id=LEONARDO_ORG_ID, appointment_id=created_id, reason="sandbox cleanup")
            deleted = True
